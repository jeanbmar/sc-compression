...
const readSignature = (buffer) => {
  ...
  if (buffer.slice(0, 2).toString('utf8').toLowerCase() === 'sc') {
    ...
    if (
      buffer.length >= 34 &&
      buffer.slice(30, 34).toString('hex') === '28b52ffd'
    ) {
      return 'zstd';
    }
...
const decompress = async (buffer) => {
  ...
  if (signature === 'sc') {
    ...
    decompressed = await lzham.decompress(buffer.slice(35), {
      dictSizeLog2,
      outputSize,
    });
  }
  if (['lzma', 'sc', 'sc2', 'sig'].includes(signature)) {
    ...
    decompressed = await lzma.decompress(padded);
  }
  if (signature === 'zstd') {
    decompressed = await zstd.decompress(buffer);
  }
  if (rest != null) {
    decompressed = Buffer.concat([decompressed, rest]);
  }
  return decompressed;
};
